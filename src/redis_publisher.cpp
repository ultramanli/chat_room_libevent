...
bool RedisPublisher::init() {
	...
	int ret = sem_init(&eventSem, 0, 0);      //初始化信号量 参数:信号量,线程间共享,初始值
	if (ret != 0) {
...
bool RedisPublisher::publish(const std::string &channel_name,
	...
	int ret = redisAsyncCommand(context,
								&RedisPublisher::command_callback,
								this,
								"PUBLISH %s %s",
...
// 消息接收回调函数 （命令的返回，也就是set、get之后返回的内容）
void RedisPublisher::command_callback(redisAsyncContext *redis_context,
                                      void *reply, void *privdata) {
	std::cout<<"消息发布成功,接收订阅者数量："<<((redisReply *)reply)->integer<<std::endl;
...
void *RedisPublisher::event_proc() {
	//在connect()完成之前会阻塞，直到connect里的sem_post执行了为止
	sem_wait(&eventSem);
